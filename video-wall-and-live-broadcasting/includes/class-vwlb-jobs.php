...
final class VWLB_Jobs {
	...
	public static function reconcile(){
		...
		foreach($events as $event){$provider=VWLB_Providers::get($event['provider']);if(!$provider)continue;$start=microtime(true);$state=$provider->reconcile('live',$event);$ms=(int)round((microtime(true)-$start)*1000);VWLB_Observability::record_provider($event['provider'],'live',is_array($state)?'healthy':'degraded',is_wp_error($state)?$state->get_error_code():'',$ms);if(!is_array($state))continue;
			$fresh=VWLB_Repository::find('live_events',$event['id']);if(!$fresh||$fresh['provider']!==$event['provider']||!in_array($fresh['status'],array('scheduled','live','interrupted'),true))continue;$safe=array_intersect_key($state,array_flip(array('provider_event_ref','status','degraded','region','health_code')));$merged=array_merge(VWLB_Helpers::json($fresh['provider_state_json']),$safe);$persisted=VWLB_Repository::update_versioned('live_events',$fresh['id'],$fresh['version'],array('provider_state_json'=>VWLB_Helpers::json_encode($merged)));if(is_wp_error($persisted))do_action('vwlb_operational_failure','live_reconcile','vwlb_live_reconcile_persist_failed',array('live_public_id'=>$fresh['public_id']??'','code'=>$persisted->get_error_code()));
		}
	}
