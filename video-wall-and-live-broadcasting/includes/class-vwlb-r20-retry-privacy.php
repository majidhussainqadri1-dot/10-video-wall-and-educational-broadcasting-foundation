<?php
/** R20/R29/R52 sequential review: encrypted, expiring, erasable and starvation-safe inbound retry evidence. */
...
final class VWLB_R20_Retry_Privacy {
	...
	const RECONCILE_CURSOR_OPTION = 'vwlb_retry_reconcile_cursor';
	const CLEANUP_CURSOR_OPTION = 'vwlb_retry_cleanup_cursor';
	...
	private static function records($after_id=0,$limit=100){
		global $wpdb;$like=$wpdb->esc_like(VWLB_Review_Hardening::INBOX_RETRY_PREFIX).'%';$wpdb->last_error='';$rows=$wpdb->get_results($wpdb->prepare("SELECT option_id,option_name,option_value FROM {$wpdb->options} WHERE option_name LIKE %s AND option_id>%d ORDER BY option_id ASC LIMIT %d",$like,max(0,(int)$after_id),max(1,min(500,(int)$limit))),ARRAY_A);if(''!==(string)$wpdb->last_error){do_action('vwlb_operational_failure','inbox','vwlb_retry_option_read_failed',array());return null;}return is_array($rows)?$rows:array();
	}
	private static function persist_cursor($option,$value,$phase){$value=max(0,(int)$value);if(0===$value){$deleted=delete_option($option);if(!$deleted&&false!==get_option($option,false)){do_action('vwlb_operational_failure','inbox','vwlb_retry_cursor_persist_failed',array('phase'=>$phase,'action'=>'reset'));return false;}return true;}$saved=update_option($option,$value,false);if(!$saved&&(int)get_option($option,0)!==$value){do_action('vwlb_operational_failure','inbox','vwlb_retry_cursor_persist_failed',array('phase'=>$phase,'action'=>'advance'));return false;}return true;}
	...
	public static function erase_retry_evidence($email,$page=1){
		$user=get_user_by('email',$email);if(!$user)return array('items_removed'=>false,'items_retained'=>false,'messages'=>array(),'done'=>true);$uid=(int)$user->ID;$cursor_key=self::ERASURE_CURSOR_PREFIX.hash('sha256',(string)$uid);$cursor=max(0,(int)get_option($cursor_key,0));$rows=self::records($cursor,200);if(null===$rows)return array('items_removed'=>false,'items_retained'=>true,'messages'=>array(__('Secure retry evidence could not be enumerated safely.',VWLB_TEXT_DOMAIN)),'done'=>false);$removed=false;$last=$cursor;
