...
final class VWLB_Activator {
	public static function activate() {
		if ( version_compare(PHP_VERSION,'8.1','<') ) { deactivate_plugins(plugin_basename(VWLB_FILE)); wp_die(esc_html__('File 10 requires PHP 8.1 or newer.',VWLB_TEXT_DOMAIN)); }
		VWLB_DB::install_schema(); self::capabilities(); self::pages(); self::schedules(); VWLB_Compatibility::migrate_legacy();
	...
	private static function capabilities() {
		$roles=array('administrator'=>array(VWLB_Contracts::CAP_SUBMIT,VWLB_Contracts::CAP_PUBLISH,VWLB_Contracts::CAP_BROADCAST,VWLB_Contracts::CAP_MODERATE,VWLB_Contracts::CAP_MANAGE,VWLB_Contracts::CAP_DIAGNOSTICS));
	...
	private static function pages() {
		$pages=array('video-wall'=>array('title'=>__('Video Wall',VWLB_TEXT_DOMAIN),'content'=>'[vwlb_wall]'),'video'=>array('title'=>__('Video',VWLB_TEXT_DOMAIN),'content'=>'[vwlb_video]'),'live'=>array('title'=>__('Live',VWLB_TEXT_DOMAIN),'content'=>'[vwlb_live]'),'channel'=>array('title'=>__('Channel',VWLB_TEXT_DOMAIN),'content'=>'[vwlb_channel]'),'studio-video'=>array('title'=>__('Video Studio',VWLB_TEXT_DOMAIN),'content'=>'[vwlb_studio_video]'),'studio-live'=>array('title'=>__('Live Studio',VWLB_TEXT_DOMAIN),'content'=>'[vwlb_studio_live]'),'video-history'=>array('title'=>__('Video History',VWLB_TEXT_DOMAIN),'content'=>'[vwlb_history]'));
