...
final class VWLB_REST {
	...
	private function register_namespace($n){
		...
		$this->route($n,'/moderation/reports','POST','report','login');$this->route($n,'/moderation/reports/(?P<id>[A-Za-z0-9_-]+)/decision','POST','decide_report','moderate');
	...
	public function publish_video(WP_REST_Request $r){$d=$this->body($r);$v=VWLB_Videos::publish($r['id'],absint($d['version']??0),$d['scheduled_at']??null);return $this->response(is_wp_error($v)?$v:VWLB_Repository::video_mutation_dto($v));}
	...
	public function create_channel(WP_REST_Request $r){$v=VWLB_Videos::create_channel($this->body($r));return $this->response(is_wp_error($v)?$v:VWLB_Repository::channel_mutation_dto($v),201);}
	...
	public function schedule_live(WP_REST_Request $r){
		...
		else{$d['recording_policy']=is_array($d['recording_policy'])?$d['recording_policy']:array();$d['recording_policy']['record']=!empty($d['recording_policy']['record']);$d['recording_policy']['consent_required']=array_key_exists('consent_required',$d['recording_policy'])?(bool)$d['recording_policy']['consent_required']:true;}$d['recording_policy']['consent_version']=VWLB_Helpers::text($d['recording_policy']['consent_version']??'v1',64);
		...
		$result=VWLB_Live::schedule($d,$this->idem($r));if(!is_wp_error($result)&&!empty($result['id'])){$extras=VWLB_Extensions::schedule_live_extras($result['id'],$d);if(is_wp_error($extras))return $this->response($extras);$fresh=VWLB_Repository::find('live_events',$result['id']);if($fresh)$result['version']=(int)$fresh['version'];if(!empty($selection['failover'])){VWLB_Helpers::audit('live',$result['id'],'provider_failover',$preferred,$selection['provider'],'Explicit provider failover selected during scheduling.');$result['provider_failover']=$selection;}$safe=$fresh?VWLB_Repository::live_mutation_dto($fresh):array('public_id'=>$result['public_id']??'','status'=>$result['status']??'scheduled','version'=>(int)($result['version']??1));if(!empty($result['provider_failover']))$safe['provider_failover']=$result['provider_failover'];$result=$safe;}return $this->response($result,201);
	}
	...
	public function transition_live(WP_REST_Request $r){$d=$this->body($r);$v=VWLB_Live::transition($r['id'],$d['status']??'',absint($d['version']??0),$d['note']??'',$d['provider_proof']??array());return $this->response(is_wp_error($v)?$v:VWLB_Repository::live_mutation_dto($v));}
	public function kill_live(WP_REST_Request $r){$d=$this->body($r);$v=VWLB_Live::kill($r['id'],absint($d['version']??0),$d['reason']??'emergency_end');return $this->response(is_wp_error($v)?$v:VWLB_Repository::live_mutation_dto($v));}
	...
	public function transition_takedown(WP_REST_Request $r){$d=$this->body($r);$v=VWLB_Moderation::transition_takedown($r['id'],$d['status']??'',absint($d['version']??0),$d['reason']??'');return $this->response(is_wp_error($v)?$v:VWLB_Repository::takedown_mutation_dto($v));}
