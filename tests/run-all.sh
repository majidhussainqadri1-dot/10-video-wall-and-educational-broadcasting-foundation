...
text = text.replace('need \'Review boundary: final sequential cycle round `R40`\' "$ROOT/MANIFEST.md" r40-manifest-boundary', 'need \'R101–R120 sequential cycle is QA-closed.\' "$ROOT/MANIFEST.md" r40-manifest-boundary')
pathlib.Path(dst).write_text(text)
PY
  bash "$tmp"; rm -f "$tmp"
}
...
bash "$ROOT/tests/file10-r117-contracts.sh"
