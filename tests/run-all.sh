...
text = text.replace('R60 remains pending', 'R105 exact-head QA:')
pathlib.Path(dst).write_text(text)
PY
  bash "$tmp"; rm -f "$tmp"
}
...
text = text.replace('R40 found additional package/release-hygiene defects', 'R105 correction:')
pathlib.Path(dst).write_text(text)
PY
  bash "$tmp"; rm -f "$tmp"
}
