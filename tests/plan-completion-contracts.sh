...
need "Version: 1.1.0-rc1" "$P/video-wall-and-live-broadcasting.php" version
...
for token in "class VWLB_Extensions" "class VWLB_Podcasts" "class VWLB_Observability" "class VWLB_Extended_REST"; do need "$token" "$P/includes" "$token"; done
for table in upload_sessions chapters live_attendees live_questions live_resources download_tokens creator_metrics_daily provider_health premieres podcast_series podcast_episodes; do need "$table" "$P/includes" "table-$table"; done
