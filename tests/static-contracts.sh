...
MAIN="$PLUGIN/video-wall-and-live-broadcasting.php"
fail(){ echo "FAIL: $*" >&2; exit 1; }
[[ -f "$MAIN" ]] || fail "main plugin missing"
VERSION="$(sed -n "s/.*define( 'VWLB_VERSION', '\([^']*\)' ).*/\1/p" "$MAIN" | head -n1)"
[[ -n "$VERSION" ]] || fail "version constant missing"
grep -F "Version: $VERSION" "$MAIN" >/dev/null || fail version
grep -F "Stable tag: $VERSION" "$PLUGIN/readme.txt" >/dev/null || fail stable-tag
grep -F "define( 'VWLB_FUTURE_SCHEMA_VERSION', '1.2.0' );" "$MAIN" >/dev/null || fail future-schema
grep -F "Text Domain: video-wall-live-broadcasting" "$MAIN" >/dev/null || fail text-domain
...
echo "static contracts PASS ($VERSION)"
