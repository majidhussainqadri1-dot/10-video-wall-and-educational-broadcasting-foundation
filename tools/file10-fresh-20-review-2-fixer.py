...
repl(PLAN,'1.2.2-rc1','1.2.3-rc1','R20 plan completion release identity')
...
ls=LEDGER.read_text()
entry='''## R20 — DEFECT FIXED\nThe final fresh release-hygiene review found that the working corrective branch was explicitly the `v1.2.3-rc1` candidate, but runtime metadata, default package naming, release workflow artifact naming and active release-identity regression contracts still advertised `1.2.2-rc1`. Shipping the reviewed corrections under the prior candidate identity would make package/deployment parity evidence ambiguous. Runtime, deterministic builder, GitHub release QA artifact naming and all active release-identity regression expectations are now synchronized to `1.2.3-rc1`; schema versions remain unchanged because this corrective cycle did not introduce a new database schema. Two initial R20 validation attempts exposed stale release-identity assertions in the static and plan-completion suites; both QA-only drifts were corrected within R20 before any R20 product changes were accepted.\n\n'''
