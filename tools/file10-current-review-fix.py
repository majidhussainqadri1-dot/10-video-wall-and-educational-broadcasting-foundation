...
if old not in d: raise SystemExit('R36 base install pattern missing')
d=d.replace(old,new,1);db.write_text(d)
...
for path, option_expr, schema_expr in [(ext,"self::OPTION","VWLB_EXT_SCHEMA_VERSION"),(fut,"self::OPTION","self::SCHEMA")]:
    ...
    if old not in t: raise SystemExit('R36 extension/future install pattern missing: '+path.name)
    t=t.replace(old,new,1)
    # ensure method explicitly succeeds after audit if it has one, else before closing method.
    if path==fut:
        ...
        if old_a not in t: raise SystemExit('R36 future return pattern missing')
        t=t.replace(old_a,new_a,1)
    else:
        # Extension install has an audit/version tail; insert return true before next method by replacing first close after update block audit marker if present.
        marker="update_option( self::OPTION"
        pos=t.find(marker)
        end=t.find("\n\t}\n",pos)
...
if old not in a: raise SystemExit('R36 activator reconciliation pattern missing')
a=a.replace(old,new,1);act.write_text(a)
