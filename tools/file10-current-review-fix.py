...
root=Path('.')
test=Path('tests/fresh-40-review-adversarial.sh')
run=Path('tests/run-all.sh')
...
r=run.read_text()
line='bash "$ROOT/tests/fresh-40-review-adversarial.sh"\n'
if line not in r:
    anchor='bash "$ROOT/tests/fresh-40-review-contracts.sh"\n'
    if anchor not in r: raise SystemExit('R38 run-all anchor missing')
    r=r.replace(anchor,anchor+line,1)
run.write_text(r)

ledger=reg.read_text()
marker="""# R38 — permanent adversarial QA forbids recurrence of corrected unsafe patterns and verifies the sequential defect ledger.\nneed \"fresh 40-review adversarial contracts PASS\" \"$ROOT/tests/fresh-40-review-adversarial.sh\" r38-adversarial-suite\nneed \"fresh-40-review-adversarial.sh\" \"$ROOT/tests/run-all.sh\" r38-run-all\n"""
if '# R38 —' not in ledger:
    ledger=ledger.replace("printf '%s\\n' 'fresh 40-review regression contracts PASS'\n",marker+"printf '%s\\n' 'fresh 40-review regression contracts PASS'\n")
reg.write_text(ledger)
